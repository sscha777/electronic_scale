...
ft_bool_t Esd_Calibrate(EVE_HalContext *phost)
{
	...
	EVE_CoCmd_text(phost, (phost->Width / 2), (phost->Height / 2), 27, OPT_CENTER, "Please Tap on the dot");
 ...
 * @return Flash_Cmd_Status_t 
 */
Flash_Cmd_Status_t FlashHelper_Write(EVE_HalContext *phost, uint32_t dest_flash, uint32_t num, uint8_t *write_data)
{
	uint32_t i;
...
#endif /* #if (EVE_SUPPORT_CHIPID >= EVE_BT817) */
 ...
 * @return ft_void_t 
 */
ft_void_t FlashHelper_flashWriteExt(EVE_HalContext *phost, uint32_t dest, uint32_t num, uint8_t *data)
{
...
{
	uint8_t status = Gpu_Hal_Rd8(host, REG_FLASH_STATUS);

	if (status == FLASH_STATUS_DETACHED)
	{
		Gpu_CoCmd_FlashAttach(host);
		App_Flush_Co_Buffer(host);
		Gpu_Hal_WaitCmdfifo_empty(host);
		status = Gpu_Hal_Rd8(host, REG_FLASH_STATUS);
	...
	int32_t size = Gpu_Hal_Rd32(host, REG_FLASH_SIZE);
