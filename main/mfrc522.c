...
/**
 * @brief mfrc522 UART 드라이버
 *
 */

uint8_t cardSerial[4];
uint8_t returnBlock[16] = {
    0,
};
...
#define MFRC_RST_PIN (GPIO_NUM_27)

// #define GPIO_OUTPUT_IO_0    18
// #define GPIO_OUTPUT_PIN_SEL  ((1ULL<<GPIO_OUTPUT_IO_0))
#define GPIO_INPUT_IO_0 GPIO_NUM_25
#define GPIO_INPUT_PIN_SEL ((1ULL << GPIO_INPUT_IO_0))
#define GPIO_OUTPUT_PIN_SEL ((1ULL << MFRC_RST_PIN))
...
void mfrc_tx_init(void)
{

    uint8_t txByte;

    txByte = rc522_uart_set(0x01, 0x0f); // software reset
    DELAY_MS(30);
    txByte = rc522_uart_set(0x0c, 0x10); // ControlReg - Initiator
    DELAY_MS(10);
    txByte = rc522_uart_set(0x15, 0x40); // TX auto reg
    DELAY_MS(10);
    txByte = rc522_uart_set(0x18, 0x55); // TX auto reg
    DELAY_MS(10);
    txByte = rc522_uart_set(0x19, 0x4d); // DemodReg
    DELAY_MS(30);
    txByte = rc522_uart_set(0x26, 0x59); // RFCfgReg - RxGain
    DELAY_MS(10);
    txByte = rc522_uart_set(0x27, 0xf4); // GsNReg - CWGsN, ModGsN
    DELAY_MS(10);
    txByte = rc522_uart_set(0x14, 0x83); // TxControlReg - InvTx2On=1, Tx2RFEn, Tx1RFEn
    DELAY_MS(10);
    txByte = txByte;
}
...
esp_err_t rc522_uart_set(uint8_t addr, uint8_t tx_data)
{
    ...
    int8_t txd = 0;
    key[0] = addr; // CommandReg - transceive
    key[1] = tx_data;
    txd = uart_write_bytes(UART_NUM_1, key, 2);
    if (txd == -1)
    {
        // ////ESP_LOGE(TAG, "uart set error!");
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
esp_err_t rc522_uart_req(uint8_t addr, uint8_t dat, TickType_t ticks_to_wait)
{
    ...
    txByte = rc522_uart_getchar(addr);

    if (txByte != dat)
    {
        // ////ESP_LOGE(TAG, "req data error[%x]", txByte);
        return ESP_FAIL;
    }
    ESP_LOGD(TAG, "req[%x]", txByte);
    return ESP_OK;
}
...
uint8_t rc522_uart_getchar(uint8_t addr)
{
    ...
    txaddr[0] = addr | 0x80; // 워닝 방지
    uart_flush(UART_NUM_1);
    txByte = uart_write_bytes(UART_NUM_1, txaddr, 1);
    txByte = txByte;
    DELAY_MS(10);
    for (int i = 0; i < 5; i++)
    {
        if (uart_read_bytes(UART_NUM_1, &buf, 1, 50 / portTICK_PERIOD_MS))
        {
            // ESP_LOGD(TAG, "get[%x][%x] try[%d]", addr, buf[0], i);
            return buf[0];
        }
    }
    // ESP_LOGD(TAG, "getchar error[%x][%x]", addr, buf[0]);
    return 0xff;
}
...
void scan_tag(void)
{
    ...
    rc522_uart_set(0x01, 0x0c); // CommandReg - transceive
    rc522_uart_set(0x0a, 0x80); // flush FIFO
    DELAY_MS(10);
    rc522_uart_set(0x09, 0x26); //  FIFO - Request code
    DELAY_MS(10);
    if (ESP_OK != rc522_uart_req(0x0a, 0x01, (50 / portTICK_PERIOD_MS))) // Read FIFOLevel
    {
        // ////ESP_LOGE(TAG, "0x0a,0x01 REQ error");
        return;
    }

    rc522_uart_set(0x0d, 0x87); // BitframingReg - StartSend, TxLastBits
    DELAY_MS(10);
    //> Response ATQ:
    rxByte[0] = rc522_uart_getchar(0x06); // Read ErrReg
    DELAY_MS(10);
    if (ESP_OK != rc522_uart_req(0x0a, 0x02, (50 / portTICK_PERIOD_MS))) // Read FIFOLevel
    {
        // ////ESP_LOGE(TAG, "0x0a,0x02 REQ error");
        return;
    }
    rxByte[0] = rc522_uart_getchar(0x09); // ATQ LSB
    rxByte[0] = rc522_uart_getchar(0x09); // ATQ MSB

    //> Mifare Anticoll:
    rc522_uart_set(0x0a, 0x80); //  flush FIFO
    rc522_uart_set(0x09, 0x93); //  fast activation of Mifare
    rc522_uart_set(0x09, 0x20); //  NVB byte (nothing known)
    rc522_uart_set(0x0d, 0x80); //  BitframingReg - StartSend, TxLastBits

    DELAY_MS(10);
    //<Response SNR:
    rxByte[0] = rc522_uart_getchar(0x06);                                // Read ErrReg
    if (ESP_OK != rc522_uart_req(0x0a, 0x05, (50 / portTICK_PERIOD_MS))) // Read FIFOLevel
    {
        // ////ESP_LOGE(TAG, "0x0a,0x05 REQ error");
        return;
    }
    rxByte[0] = rxByte[0];
    taskInfo.tag_serial[0] = rc522_uart_getchar(0x09);
    taskInfo.tag_serial[1] = rc522_uart_getchar(0x09);
    taskInfo.tag_serial[2] = rc522_uart_getchar(0x09);
    taskInfo.tag_serial[3] = rc522_uart_getchar(0x09);
    taskInfo.tag_serial[4] = rc522_uart_getchar(0x09);
    //ESP_LOGI(TAG, "TAG = %x %x %x %x %x", taskInfo.tag_serial[0], taskInfo.tag_serial[1], taskInfo.tag_serial[2], taskInfo.tag_serial[3], taskInfo.tag_serial[4]);
    rc522_uart_set(0x0d, 0x80); //  BitframingReg - StartSend, TxLastBits
    DELAY_MS(10); // sync error
    taskInfo.tag_flag = 1;
    return;
}
...
{
    // uint8_t returnLen = 0;
    //gpio_pad_select_gpio(MFRC_RST_PIN);
    esp_rom_gpio_pad_select_gpio(MFRC_RST_PIN);
    gpio_set_direction(MFRC_RST_PIN, GPIO_MODE_OUTPUT);
    gpio_set_level(MFRC_RST_PIN, 0);
    DELAY_MS(1);
    gpio_set_level(MFRC_RST_PIN, 1);
    DELAY_MS(7000);
